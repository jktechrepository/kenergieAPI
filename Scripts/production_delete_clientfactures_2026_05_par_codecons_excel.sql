...
--   « Factures non conforme MAI 2.xlsx » (163 codes uniques après dédoublonnage).
--
-- Relation :
...
-- Charger les 163 CodeCons (généré depuis l'Excel) :
...
SELECT
    ...
    (SELECT COUNT(DISTINCT LOWER(TRIM(t.CodeCons)))
     FROM tmp_codecons_cibles t
     INNER JOIN Clients c ON LOWER(TRIM(c.CodeCons)) = LOWER(TRIM(t.CodeCons))) AS nb_codecons_resolus,
    (SELECT COUNT(*)
     FROM tmp_codecons_cibles t
     LEFT JOIN Clients c ON LOWER(TRIM(c.CodeCons)) = LOWER(TRIM(t.CodeCons))
